...
app.get("/line2_1", async (req, res) => {
  try {
    ...
    const { Description: spDescription, "Price History": spPriceHistory } =
      SP500;
    ...
    const bodyTW = await dbTW.view("sentiment", "sentiment2_1", {
      reduce: true,
      group: true,
    });
...
app.get("/line3_1", async (req, res) => {
  ...
  try {
    const bodyTW = await dbTW.view("sentiment", "sentiment_housing", {
      reduce: true,
      group: true,
    });
...
function fetchDataAndSendResponse() {
  app.get("/bar", async (req, res) => {
    ...
    const totalSentimentTW = bodyTW.rows.reduce(
    ...
    const totalSentimentAus = bodyMaAus.rows.reduce(
      (sum, row) => sum + parseFloat(row.value),
      0
    );
    ...
    const totalSentimentUs = bodyMaAus.rows.reduce(
      (sum, row) => sum + parseFloat(row.value),
      0
    );
    ...
    const totalSentimentAfr = bodyMaAfr.rows.reduce(
      (sum, row) => sum + parseFloat(row.value),
      0
    );
