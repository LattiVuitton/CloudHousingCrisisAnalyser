...
const dbTW = couchdb.db.use("twitter_data_test");
...
app.get("/bar", async (req, res) => {
  ...
  console.log("bodyTW sentiment", bodyTW.total_rows);
  const countMapTW = new Map();
  let total = 0.0;
  bodyTW.rows.map((doc) => {
    //console.log("doc ", doc);
    const { key } = doc;
    total = total + parseFloat(key);
  });
  console.log("totalTW: ", total);
  countMapTW.set("average", total / parseFloat(bodyTW.total_rows));

  console.log("average of TW sentiments: ", countMapTW.get("average"));

  //Ma AUS
  const bodyMaAus = await dbMA.view("sentiment", "sentimentAus");
  console.log("bodyMaAus sentiment", bodyMaAus.total_rows);
  const countMapMAAus = new Map();
  let totalMAAus = 0.0;
  bodyMaAus.rows.map((doc) => {
    //console.log("doc ", doc);
    const { key } = doc;
    totalMAAus = totalMAAus + parseFloat(key);
  });
  countMapMAAus.set("average", totalMAAus / parseFloat(bodyMaAus.total_rows));
  ...
  console.log("bodyMaAfr sentiment", bodyMaAfr.total_rows);
  const countMapMAAfr = new Map();
  let totalMAAfr = 0.0;
  bodyMaAfr.rows.map((doc) => {
    //console.log("doc ", doc);
    const { key } = doc;
    totalMAAfr = totalMAAfr + parseFloat(key);
  });
  countMapMAAfr.set("average", totalMAAfr / parseFloat(bodyMaAfr.total_rows));
      ...
      data: [
        {
          ...
          Sentiment: countMapTW.get("average"),
        },
        {
          "Platform/Country": "Mastodon Australia",
          Sentiment: countMapMAAus.get("average"),
        },
        {
          "Platform/Country": "Mastodon US",
          Sentiment: countMapMUs.get("average"),
        },
        {
          "Platform/Country": "Mastodon Africa",
          Sentiment: countMapMAAfr.get("average"),
        },
      ],
    });
  } catch (error) {
